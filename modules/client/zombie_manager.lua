        ...
        while true do
            ...
            for zoneName, zoneData in pairs(self.ZoneCache) do
                ...
                if distance <= Config.MaxRenderDistance then
                    local maxZombies = math.floor((zoneData.maxZombies or Config.DefaultZoneSettings.MaxZombies) * spawnMultiplier)
...
function ZombieManager:CreateZombie(coords, zombieType)
    ...
    -- Vision & Hearing
    local visualRange = self:IsNightTime() and Config.ZombieAggroSettings.visualRangeNight or Config.ZombieAggroSettings.visualRange
    ...
    if isAggro then
        ...
        if not IsPedInCombat(zombie.entity, playerPed) then
            ...
            if Config.ZombieBehavior.WanderEnabled then
                local wanderInterval = math.random(Config.ZombieBehavior.WanderInterval.min, Config.ZombieBehavior.WanderInterval.max)
...
function ZombieManager:MakeZombieWander(zombie, currentPos)
    ...
    if found then
        ...
        TaskGoToCoordAnyMeans(zombie.entity, targetPos.x, targetPos.y, targetPos.z, Config.ZombieBehavior.WanderSpeed, 0, false, 786603, 0.0)
        ...
        if Config.Debug then
            ...
            CreateThread(function()
                ...
                while GetGameTimer() < endTime do
                    DrawLine(currentPos.x, currentPos.y, currentPos.z + 1.0, targetPos.x, targetPos.y, targetPos.z + 1.0, 0, 255, 0, 100)
                    Wait(0)
                end
            end)
        end
